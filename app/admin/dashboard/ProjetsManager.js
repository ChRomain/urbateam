...
export default function ProjetsManager({ role }) {
  ...
  const handleGeocode = async (e) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        showToast('Localisation trouvée !', 'success');
      } else {
  ...
  const handleSubmit = async (e, forcedStatus = null) => {
    ...
    try {
      ...
      if (result.success) {
        setMessage({ type: 'success', text: status === 'draft' ? 'Réalisation enregistrée en brouillon' : (editingProject ? 'Réalisation mise à jour !' : 'Réalisation publiée !') });
        if (!editingProject) e.target.reset();
        ...
        <div className="lg:col-span-5">
          <GlassCard style={{ padding: '2rem' }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                ...
                <div className="form-group">
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <input 
                      ...
                      style={{ flex: 1, padding: '0.8rem', borderRadius: '10px', border: '1px solid #e2e8f0', fontSize: '0.9rem' }} 
                    />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2" style={{ gap: '1rem' }}>
                <div className="form-group">
                  <label style={{ fontSize: '0.8rem', color: '#64748b', fontWeight: '600', marginBottom: '0.4rem', display: 'block' }}>Latitude (auto)</label>
                  <input 
                    name="latitude" 
                    type="number" 
                    step="any"
                    ...
                    placeholder="48.44..." 
                    style={{ width: '100%', padding: '0.6rem', borderRadius: '8px', border: '1px solid #e2e8f0', fontSize: '0.8rem' }} 
                  />
                </div>
                <div className="form-group">
                  <label style={{ fontSize: '0.8rem', color: '#64748b', fontWeight: '600', marginBottom: '0.4rem', display: 'block' }}>Longitude (auto)</label>
                  <input 
                    name="longitude" 
                    type="number" 
                    step="any"
